...
def get_model(model_name:str, num_outs:int=2)->torch.nn.Module:
    if model_name == "inception":
        md = models.Inception3(init_weights=models.Inception_V3_Weights.DEFAULT)
        md.fc = nn.Sequential(
            nn.Dropout(.25),
            nn.Linear(2048,num_outs),
        )
        return md
    if model_name == "vgg16":
...
if __name__ == "__main__":
    ...
    with torch.inference_mode():
        get_model(args.model)(torch.randn(1,3,256,256))
